...
void HitView::BuildDisplay(TCanvas *c)
{
	myCanvas=c;
...
void HitView::DrawEvent(ParticleObjectHolder * poh,  NtpStRecord *ntp,int ntpEvt)
{
	padU->Clear();
	padV->Clear();
	padMC->Clear();
	...
	viewU = new TH2D("hu","Hits U", nz, minz,maxz,nu,minu, maxu);
	viewV = new TH2D("hv","Hits V", nz, minz,maxz,nv,minv, maxv);
	viewU->SetStats(0);
	viewV->SetStats(0);
	...
	{	
		...
		DrawChains(2);
		if(isMC)DrawTruthVectors(ntp,ntpEvt,2);
		...
		DrawChains(3);	
		if(isMC)DrawTruthVectors(ntp,ntpEvt,3);
...
void HitView::DrawTruthVectors(NtpStRecord *str, int index, int view)
{
	...
	vector<const NtpMCStdHep *> hep = str->GetMCStdHeps(index);
  ...
  for(int i=0;i<cnt;i++){
        ...
        if(hep[toUse]->IstHEP!=1)continue;
        
		double vt=view==2?mypoh->mctrue.vtx_u:mypoh->mctrue.vtx_v;
		double vz = mypoh->mctrue.vtx_z;
		
		//scale for length of line
		double scale=0.1;
		
	/*	double ptot = sqrt(hep[toUse]->p4[0]*hep[toUse]->p4[0]+hep[toUse]->p4[1]*hep[toUse]->p4[1]+hep[toUse]->p4[2]*hep[toUse]->p4[2]);
		
		double angu = atan(hep[toUse]->p4[0]/hep[toUse]->p4[2]);
		double angv = atan(hep[toUse]->p4[1]/hep[toUse]->p4[2]);
		
		double nx = ptot*scale*sin(angu);
		double ny = ptot*scale*sin(angv);
		double nz = ptot*scale;
	*/
	
		double nx = scale*hep[toUse]->p4[0];
		double ny = scale*hep[toUse]->p4[1];
		double nz = scale*hep[toUse]->p4[2];
		
		double eu=1/sqrt(2.0)*(nx+ny);
		double ev=1/sqrt(2.0)*(-nx+ny);
		

		double et=view==2?eu:ev;
		et+=vt;
		double ez=vz + nz;
		

		
		TLine *l = new TLine(vz,vt,ez,et);
...
void HitView::DrawChains(int view)
{       
...
void HitView::DrawClusters(int view, TH2 * h)
{
     ...
     std::map<double, int>::iterator s_iterr;
	    ...
	    for(unsigned int i=0;i<mypoh->strips.hits.size();i++)
	    {
	    	if(mypoh->strips.hits[i].view!=view)continue;
	    	if(mypoh->strips.hits[i].e<0.3)continue;
	    	h->Fill(mypoh->strips.hits[i].z,mypoh->strips.hits[i].t,mypoh->strips.hits[i].e);
...
void HitView::DrawVertex(int view)
{
...
void HitView::DrawTrueVertex(int view)
{
...
void HitView::DrawInteractionDiagram(NtpStRecord *str,int index)
{
...
void HitView::DrawBasicInfo(NtpStRecord *str,int index)
{
