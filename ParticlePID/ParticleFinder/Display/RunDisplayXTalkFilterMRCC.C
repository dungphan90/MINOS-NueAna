...
void RunDisplayXTalkFilterMRCC(string sntp_file="",string mrnt_file="")
{
 ...
 JobCPath &jp= jc->Path.Create("default","XTalkFilter::Reco ParticleFinder::Reco  ParticleAna::Reco ParticleDisplay::Ana");
 jc->Input.Set("Format=input");
  jc->Input.Set("Streams=PO,PA,NtpSt");

jp.Mod("ParticleFinder").Set("DoMRCC=1");
jp.Mod("ParticleAna").Set("DoMRCC=1");
