...
void RunDisplayXTalkFilterSysGains(string sntp_file="",string mrnt_file="")
{
  ...
  gROOT->LoadMacro("NueAna/ParticlePID/ParticleFinder/macros/LoadLibs.C"); LoadDisplayLibs();
...
gSystem->Load("libXTalkFilter.so");
  ...
  jc->Msg.SetLevel("ShwFit","Error");
   jc->Msg.SetLevel("SystematicGains","Debug");
  ...
  jc->Path.Create("default","SystematicGains::Reco XTalkFilter::Reco ParticleFinder::Reco  ParticleAna::Reco ParticleDisplay::Ana");
 ...
 char tmpchar[200]; 
sprintf(tmpchar,"RandomShift=%f",0.7);
printf("%s\n",tmpchar);
jc->Path("default").Mod("SystematicGains").Set(tmpchar);
sprintf(tmpchar,"TotalShift=%f",0.5);
printf("%s\n",tmpchar);
jc->Path("default").Mod("SystematicGains").Set(tmpchar);		
