void alltestSysGains(int nSkip=0, int nRun=-1, double allshift, double rndshift){
...
sprintf(tmpchar,"RandomShift=%f",rndshift);
j.Path("test").Mod("SystematicGains").Set(tmpchar);
sprintf(tmpchar,"TotalShift=%f",allshift);
j.Path("test").Mod("SystematicGains").Set(tmpchar);		
...
j.Path("test").Mod("Output").Set("Streams=PA,BeamMon,PO");
//j.Path("test").Mod("Output").Set("Streams=PO");
  ...
  j.Msg.SetLevel("SystematicGains","Error");
  ...
  sprintf(setfilename,"POTTreeFileName=PO-%s.root",
          file.c_str());
  j.Path("test").Mod("ParticleFinder").Set(setfilename);
  

  
  	char knobfilename[200];
  	sprintf(knobfilename,"PO-%s.root", file.c_str());
	kb->SaveSettings(knobfilename,"knobs");
