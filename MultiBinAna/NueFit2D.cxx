...
// ////////////RBT Start edits///////////// //
static void dlnlFunction(int &npar, double * /*gin*/, double &result,
...
static void globalGridFunction(int &, double * /*gin*/, double &likelihoodValue,
                               double *gridPar, int /*iflag*/) {
  std::vector<double> f;
  NueFit2D *nf2d = (NueFit2D*)ssgMinuit->GetObjectFit();
...
static void globalGridCombineFunction(int &, double * /*gin*/, double &likelihoodValue, double *gridPar, int /*iflag*/) {
  std::vector<double> f;
  NueFit2D *nf2d = (NueFit2D*)ssgMinuit->GetObjectFit();
  ...
  - f[2] = grid_sinsqth24
  - f[3] = grid_th34    (rad)
  - f[4] = grid_delta13 (rad)
  - f[5] = grid_delta14 (rad)
  - f[6] = grid_delta24 (rad)
  */

  double th14 = TMath::ASin(TMath::Sqrt(nf2d->grid_sinsqth14));
  double th24 = TMath::ASin(TMath::Sqrt(nf2d->grid_sinsqth24));
  likelihoodValue = nf2d->StdLikeComparisonForGlobalMinSterileFit(f);
  double disappChi2 = nf2d->DisappCalc.Chi2(nf2d->grid_n_dm2_32,
                                      nf2d->grid_n_dm2_21,
                                      nf2d->grid_n_th23,
                                      nf2d->grid_n_th12,
                                      nf2d->grid_n_th13,
                                      ...
                                      nf2d->grid_delta14,
                                      nf2d->grid_delta24);
  likelihoodValue += disappChi2;
  return;
}

static void dataFitFunction(int &, double * /*gin*/, double &likelihoodValue,
                            double *gridPar, int /*iflag*/) {
  std::vector<double> f;
  NueFit2D *nf2d = (NueFit2D *)ssgMinuit->GetObjectFit();
  ...
  likelihoodValue = nf2d->StdLikeComparisonForDataSterileFit(f);

  return;
}

double NueFit2D::StdLikeComparisonForDataSterileFit(vector<double> npar) {
  if (NObs == 0) {
    cout << "NObs not set.  Quitting..." << endl;
    return 0;
  }
  if (FitMethod == 1 && (FracErr_Bkgd == 0 || FracErr_Sig == 0)) {
    cout << "FracErr_Bkgd and FracErr_Sig need to be set for ScaledChi2.  "
            "Quitting..."
         << endl;
    return 0;
  }
  ...
  for (unsigned int ie = 0; ie < Extrap.size(); ie++) {
    ...
    Extrap[ie]->SetOscPar(OscPar::kDelta24, npar.at(5));
  ...
  default: {
    cout << "Error in GetMinLikelihoodSterileFit(): Unknown 'FitMethod'. "
            "BinLikelihood() is used."
         << endl;
...
double NueFit2D::DoGlobalMinSearchSterileFit(vector<double> Start, bool DoCombineFit) {
  ssgMinuit = new TMinuit(6);
  // gMinuit = ssgMinuit;
  ssgMinuit->SetObjectFit(this);
  if (!DoCombineFit) {
    ssgMinuit->SetFCN(globalGridFunction);
  } else {
    ssgMinuit->SetFCN(globalGridCombineFunction);
  }
  ...
  for (Int_t i = 0; i < 2; i++) { // SinSqTh14, SinSqTh24
    startValue[i] = Start.at(i);
    stepValue[i] = 0.01;
    /*
    if (Start.at(i) - 0.05 >= 0) {
        Bmin[i] = Start.at(i) - 0.05;
    } else {
        Bmin[i] = 0;
    }
    if (Start.at(i) + 0.05 <= 0.5){
        Bmax[i] = Start.at(i) + 0.05;
    } else {
        Bmax[i] = 0.5;
    }
    */
    Bmin[i] = 0;
    Bmax[i] = 0.5;
    ssgMinuit->mnparm(i, Form("Sin%i", i), startValue[i], stepValue[i], Bmin[i],
                      Bmax[i], iErflg);
  }
  // Theta34
  startValue[2] = Start.at(2);
  stepValue[2] = 0.05;
  Bmin[2] = 0.0;
  Bmax[2] = TMath::Pi() / 2;
  ssgMinuit->mnparm(2, Form("Theta34"), startValue[2], stepValue[2], Bmin[2],
                    Bmax[2], iErflg);

  // Delta13
  startValue[3] = Start.at(3);
  stepValue[3] = 0.05;
  Bmin[3] = 0.0;
  Bmax[3] = 2 * TMath::Pi();
  ssgMinuit->mnparm(3, Form("DeltaCP"), startValue[3], stepValue[3], Bmin[3],
                    Bmax[3], iErflg);

  // DeltaEff
  startValue[4] = Start.at(4);
  stepValue[4] = 0.05;
  Bmin[4] = 0.0;
  Bmax[4] = 2 * TMath::Pi();
  ssgMinuit->mnparm(4, Form("Delta14"), startValue[4], stepValue[4], Bmin[4],
                    Bmax[4], iErflg);
  startValue[5] = Start.at(5);
  stepValue[5] = 0.05;
  Bmin[5] = 0.0;
  Bmax[5] = 2 * TMath::Pi();
  ssgMinuit->mnparm(5, Form("Delta24"), startValue[5], stepValue[5], Bmin[5],
                    Bmax[5], iErflg);
  ...
  // Max iterations:
  ssgMinuit->SetMaxIterations(300);
  // Go minimize!
...
static void binFunction(int &npar, double * /*gin*/, double &result,
                        double *par, int /*iflag*/) {
  std::vector<double> f;
  NueFit2D *nf2d = (NueFit2D *)gMinuit->GetObjectFit();
  ...
  for (Int_t i = 0; i < npar; i++) {
    f.push_back(par[i]);
    std::cout << "f_" << i << ": " << par[i] << std::endl;
  }
  result = nf2d->BinLikeComparison(f);
  std::cout << "binFunction: " << result << std::endl;
}

static void binFunction_Separate_Syst_Stat(int &npar, double * /*gin*/, double &result,
                        double *par, int /*iflag*/) {
  std::vector<double> f;
  NueFit2D *nf2d = (NueFit2D *)gMinuit->GetObjectFit();

  // notice that npar here will be sys_size + 3
  for (Int_t i = 0; i < npar; i++) {
    f.push_back(par[i]);
  }
  double dummy_chi2_syst = 0.;
  double dummy_chi2_stat = 0.;
  double dummy_chi2_numu = 0.;
  result = nf2d->BinLikeComparison_Separate_Syst_Stat_NueNumuCombo(f, dummy_chi2_syst, dummy_chi2_stat, dummy_chi2_numu);
}

static void binFunction_Separate_Syst_Stat_LogScale(int &npar, double * /*gin*/, double &result, double *par, int /*iflag*/) {
  std::vector<double> f;
  NueFit2D *nf2d = (NueFit2D *)gMinuit->GetObjectFit();

  // notice that npar here will be sys_size
  for (Int_t i = 0; i < npar; i++) {
    f.push_back(par[i]);
  }
  double dummy_chi2_syst = 0.;
  double dummy_chi2_stat = 0.;
  double dummy_chi2_numu = 0.;
  result = nf2d->BinLikeComparison_Separate_Syst_Stat_NueNumuCombo_LogScale(f, dummy_chi2_syst, dummy_chi2_stat, dummy_chi2_numu);
}

static void binFunction_WithPulls(int &npar, double * /*gin*/, double &result,
                        double *par, int /*iflag*/) {
  std::vector<double> f;
  NueFit2D *nf2d = (NueFit2D *)gMinuit->GetObjectFit();
  ...
  double dummy_chi2_syst = 0.;
  double dummy_chi2_stat = 0.;
  result = nf2d->BinLikeComparison_WithPulls(f, dummy_chi2_syst, dummy_chi2_stat);
}
// //////////End edits///////////////////////
...
NueFit2D::NueFit2D() {
  ...
  SetOutputFile(); // s = "SensOut.root";
  SetNExperiments(); // Numexpts = 2000;

  // Combination with NumuCC disappearance
  ...
  SetFitMethod(); // Default fit method 0 (Poisson). wth is poisson?
...
void NueFit2D::ReadGridFiles() {
  ...
  if (!gSystem->AccessPathName(gSystem->ExpandPathName(
          GridFileName_Normal.c_str()))) { // if file exists
    fnorm =
        new TFile(gSystem->ExpandPathName(GridFileName_Normal.c_str()), "READ");
    for (i = 0; i < nBins; i++) {
  ...
  nPts_Inverted = 0;
  if (!gSystem->AccessPathName(gSystem->ExpandPathName(
          GridFileName_Inverted.c_str()))) { // if file exists
    finvt = new TFile(gSystem->ExpandPathName(GridFileName_Inverted.c_str()),
                      "READ");
    for (i = 0; i < nBins; i++) {
...
void NueFit2D::DefineBinDlnLMinuit() {
  ...
  // Informs Minuit that the relative floating point arithmetic precision is <accuracy>. Minuit determines the nominal precision itself, but the SET EPS command can be used to override Minuit's own determination, when the user knows that the FCN function value is not calculated to the nominal machine accuracy. Typical values of <accuracy> are between 10-5 and 10-14.
}

void NueFit2D::DefineBinDlnLMinuit_Separate_Syst_Stat() {
  // Function sets the maximum size of Minuit.
  ...
  // Make new minuit:
  minuit = new TMinuit(npar + 3); // adding the fit for theta13, theta23 and dmsq32
  minuit->SetPrintLevel(-1);
...
void NueFit2D::DefineBinDlnLMinuit_Separate_Syst_Stat_LogScale() {
  // Function sets the maximum size of Minuit.
...
void NueFit2D::SetFracError(vector<TH1D *> bkglist, vector<TH1D *> siglist) {
  ...
  for (unsigned int i = 0; i < bkglist.size(); i++) {
    FracErr_Bkgd_List.push_back(
        (TH1D *)(bkglist[i]->Clone(Form("FracErr_Bkgd_%i", i))));
    FracErr_Sig_List.push_back(
        (TH1D *)(siglist[i]->Clone(Form("FracErr_Sig_%i", i))));
  }
}
...
double NueFit2D::DoStdMinParam() {
  ...
  for (Int_t i = 0; i < sys_size; i++) {
    ...
    // if (InvErrorMatrix==0&&(unsigned int)i>=FracErr_Bkgd_List.size())
    // vstrt[i] = 1.0;
    if ((unsigned int)i >= FracErr_Bkgd_List.size()) {
      vstrt[i] = 1.0;
    }

    minuit->mnparm(i, Form("f%i", i), vstrt[i], stp[i], bmin[i], bmax[i],
                   ierflg);
...
double NueFit2D::StdLikeComparison(vector<double> npar) {
  ...
  // if (InvErrorMatrix!=0){
  //  for (unsigned int i=FracErr_Bkgd_List.size();
  //  i<FracErr_Bkgd_List.size()+nBins;i++){
  //    for (unsigned int j=FracErr_Bkgd_List.size();
  //    j<FracErr_Bkgd_List.size()+nBins;j++){
...
double NueFit2D::DoBinMinParam() {
  ...
  std::cout << "DEBUG INFO:" << std::endl;
  std::cout << "Number of MINUIT parameters: " << sys_size << std::endl;
...
double NueFit2D::DoBinMinParam_Separate_Syst_Stat(double& chi2_syst, double& chi2_stat, double& chi2_numu, std::vector<double>& npar_bestfit) {
  int sys_size = nBins;
  if (sys_size > minuit->fMaxpar) {
    cout << "WARNING: WRONG MINUIT SIZE" << endl;
  }
  if (sys_size == minuit->fMaxpar - 3) {
    cout << "INFO: WE ARE FITTING THREE-FLAVOUR PARAMETERS AS WELL!" << endl;
  }

  // The first 12 parameters are the nuisance parameters (systematics pull terms)
  // The 13th parameter is theta13.
  // The 14th parameter is theta23.
  // The 15th parameter is dmsq32.

  Double_t *vstrt = new Double_t[sys_size + 3];
  Double_t *stp = new Double_t[sys_size + 3];
  Double_t *bmin = new Double_t[sys_size + 3];
  Double_t *bmax = new Double_t[sys_size + 3];
  Int_t ierflg = 0;

  // Pass on the object and set the static function:
  gMinuit = minuit;
  minuit->SetObjectFit(this);
  minuit->SetFCN(binFunction_Separate_Syst_Stat);
  ...
  // theta 13
  // http://pdglive.lbl.gov/Particle.action?node=S067
  vstrt[sys_size] = 0.14817;
  stp[sys_size] = 0.001;
  bmin[sys_size] = 0.14577;
  bmax[sys_size] = 0.15057;
  minuit->mnparm(sys_size, Form("f%i", sys_size),
                           vstrt[sys_size],
                           stp[sys_size],
                           bmin[sys_size],
                           bmax[sys_size],
  ...
  // theta 23
  // https://minosgroup.slack.com/archives/G015Q7X2XK9/p1593375528231300?thread_ts=1593371761.203400&cid=G015Q7X2XK9
  vstrt[sys_size + 1] = 0.79584;
  stp[sys_size + 1] = 0.01;
  bmin[sys_size + 1] = 0.71274;
  bmax[sys_size + 1] = 0.87894;
  minuit->mnparm(sys_size + 1, Form("f%i", sys_size + 1),
                           vstrt[sys_size + 1],
                           stp[sys_size + 1],
                           bmin[sys_size + 1],
                           bmax[sys_size + 1],
  ...
  // dmsq32
  // https://minos-docdb.fnal.gov/cgi-bin/private/ShowDocument?docid=13747
  vstrt[sys_size + 2] = 2.445E-3;
  stp[sys_size + 2] = 0.05E-3;
  bmin[sys_size + 2] = 2.37E-3;
  bmax[sys_size + 2] = 2.52E-3;
  minuit->mnparm(sys_size + 2, Form("f%i", sys_size + 2),
                           vstrt[sys_size + 2],
                           stp[sys_size + 2],
                           bmin[sys_size + 2],
                           bmax[sys_size + 2],
  ...
  // Getting separate chi2_syst and chi2_stat
  npar_bestfit.clear();
  for (int i = 0; i < sys_size; ++i) {
    double param = 0.;
    double param_err = 0.;
    minuit->GetParameter(i, param, param_err);
    npar_bestfit.push_back(param);
  }

  double minuit_bestfit_th13 = 0.;
  double minuit_bestfit_th23 = 0.;
  double minuit_bestfit_dm32 = 0.;
  double minuit_bestfit_th13_error = 0.;
  double minuit_bestfit_th23_error = 0.;
  double minuit_bestfit_dm32_error = 0.;
  minuit->GetParameter(sys_size + 0, minuit_bestfit_th13, minuit_bestfit_th13_error);
  minuit->GetParameter(sys_size + 1, minuit_bestfit_th23, minuit_bestfit_th23_error);
  minuit->GetParameter(sys_size + 2, minuit_bestfit_dm32, minuit_bestfit_dm32_error);
  npar_bestfit.push_back(minuit_bestfit_th13);
  npar_bestfit.push_back(minuit_bestfit_th23);
  npar_bestfit.push_back(minuit_bestfit_dm32);

  chi2_syst = 0.;
  chi2_stat = 0.;
  chi2_numu = 0.;
  BinLikeComparison_Separate_Syst_Stat_NueNumuCombo(npar_bestfit, chi2_syst, chi2_stat, chi2_numu);
  // cout << "Done Migrading..." << endl;
  // Get the minimum for the function
  double minpoint = minuit->fAmin;

  if (chi2_syst + chi2_stat + chi2_numu != minpoint) {
    std::cout << "Something is wrong when calculating the chi2." << std::endl;
  }

  return minpoint;
}

double NueFit2D::DoBinMinParam_Separate_Syst_Stat_LogScale(double& chi2_syst, double& chi2_stat, double& chi2_numu, std::vector<double>& npar_bestfit) {
  int sys_size = nBins;
  if (sys_size > minuit->fMaxpar) cout << "WARNING: WRONG MINUIT SIZE" << endl;

  // The 12 parameters are the nuisance parameters (systematics pull terms)
  Double_t *vstrt = new Double_t[sys_size];
  Double_t *stp = new Double_t[sys_size];
  Double_t *bmin = new Double_t[sys_size];
  Double_t *bmax = new Double_t[sys_size];
  ...
  // gMinuit = minuit;
  minuit->SetObjectFit(this);
  minuit->SetFCN(binFunction_Separate_Syst_Stat_LogScale);
  ...
  chi2_stat = 0.;
  chi2_numu = 0.;
  BinLikeComparison_Separate_Syst_Stat_NueNumuCombo_LogScale(npar_bestfit, chi2_syst, chi2_stat, chi2_numu);
...
double NueFit2D::DoBinMinParam_WithPulls(double& chi2_syst, double& chi2_stat, std::vector<double>& npar_bestfit) {
  int sys_size = nBins;
  if (sys_size > minuit->fMaxpar) {
    cout << "WARNING: WRONG MINUIT SIZE" << endl;
  }
  ...
  gMinuit = minuit;
  minuit->SetObjectFit(this);
  minuit->SetFCN(binFunction_WithPulls);
  ...
  chi2_syst = 0.;
  chi2_stat = 0.;
  BinLikeComparison_WithPulls(npar_bestfit, chi2_syst, chi2_stat);
  // cout << "Done Migrading..." << endl;
  // Get the minimum for the function
  double minpoint = minuit->fAmin;

  if (chi2_syst + chi2_stat != minpoint) {
    std::cout << "Something is wrong when calculating the chi2." << std::endl;
  }
...
double NueFit2D::BinLikelihood_Separate_Syst_Stat(double& chi2_syst, double& chi2_stat, double& chi2_numu, std::vector<double>& npar_bestfit) {
  ...
  if (ErrCalc == 0) {
    ...
    dlnl = DoBinMinParam_Separate_Syst_Stat(chi2_syst, chi2_stat, chi2_numu, npar_bestfit);
...
double NueFit2D::BinLikelihood_Separate_Syst_Stat_LogScale(double& chi2_syst, double& chi2_stat, double& chi2_numu, std::vector<double>& npar_bestfit) {
  // Define the covariance matrix
  if (ErrCalc != 0) {
    std::cout << "Running CalculateSystErrorMatrix() ..." << std::endl;
    // Calculate the systematic error covariance matrix:
  ...
  if (ErrCalc == 0) {
    ...
    // Otherwise, do a Minuit minimization
    dlnl = DoBinMinParam_Separate_Syst_Stat_LogScale(chi2_syst, chi2_stat, chi2_numu, npar_bestfit);
...
double NueFit2D::BinLikelihood_WithPulls(double& chi2_syst, double& chi2_stat, std::vector<double>& npar_bestfit) {
  // Define the covariance matrix
  if (ErrCalc != 0) {
  ...
  if (ErrCalc == 0) {
    ...
    // Otherwise, do a Minuit minimization
    dlnl = DoBinMinParam_WithPulls(chi2_syst, chi2_stat, npar_bestfit);
...
double NueFit2D::BinLikeComparison_Separate_Syst_Stat_NueNumuCombo(vector<double> npar, double& chi2_syst, double& chi2_stat, double& chi2_numu) {
  chi2_syst = 0.0;
  chi2_stat = 0.0;
  chi2_numu = 0.0;
  double temp = 0.0;
  ...
  double minuit_param_th13 = npar.at(npar.size() - 3);
  double minuit_param_th23 = npar.at(npar.size() - 2);
  double minuit_param_dm32 = npar.at(npar.size() - 1);

  for (unsigned int ie = 0; ie < Extrap.size(); ie++) {
    Extrap[ie]->SetOscPar(OscPar::kTh13, minuit_param_th13);
    Extrap[ie]->SetOscPar(OscPar::kTh23, minuit_param_th23);
    Extrap[ie]->SetOscPar(OscPar::kDeltaM23, minuit_param_dm32);
    Extrap[ie]->OscillatePrediction();
    Bkgd->Add(Extrap[ie]->Pred_TotalBkgd_VsBinNumber);
    Sig->Add(Extrap[ie]->Pred_Signal_VsBinNumber);
  }
  ...
  if (mDoCombineFit) {
    ...
    chi2_numu = DisappCalc.Chi2(disapp_dm2_32, disapp_dm2_21, disapp_th23,
                                 disapp_th12, disapp_th13, disapp_delta13,
                                 disapp_dmsq41, disapp_th24, disapp_th34,
                                 disapp_th14, disapp_delta14, disapp_delta24);
  }
  ...
  if (npar.size() > 0 && InvErrorMatrix != 0) {
    for (unsigned int i = 0; i < npar.size(); i++) {
      for (unsigned int j = 0; j < npar.size(); j++) {
        // Covariance terms
        chi2_syst += npar.at(i) * InvErrorMatrix->GetBinContent(i + 1, j + 1) *
                npar.at(j);
      }
    }
  }

  return chi2_syst + chi2_stat + chi2_numu;
}

double NueFit2D::BinLikeComparison_Separate_Syst_Stat_NueNumuCombo_LogScale(vector<double> npar, double& chi2_syst, double& chi2_stat, double& chi2_numu) {
  chi2_syst = 0.0;
  chi2_stat = 0.0;
  chi2_numu = 0.0;
  double temp = 0.0;
...
double NueFit2D::BinLikeComparison_WithPulls(vector<double> npar, double& chi2_syst, double& chi2_stat) {
  chi2_syst = 0.0;
  chi2_stat = 0.0;
  double temp = 0.0;
  ...
  if (npar.size() > 0 && InvErrorMatrix != 0) {
    for (unsigned int i = 0; i < npar.size(); i++) {
      for (unsigned int j = 0; j < npar.size(); j++) {
        // Covariance terms
        chi2_syst += npar.at(i) * InvErrorMatrix->GetBinContent(i + 1, j + 1) * npar.at(j);
      }
    }
  }

  return chi2_syst + chi2_stat;
}
...
/* TODO: uncomment for production. This is for debugging.
*/
  std::cout << "Inverse Covariance Matrix: " << std::endl;
   if (npar.size() > 0 && InvErrorMatrix != 0) {
    for (unsigned int i = 0; i < npar.size(); i++) {
      for (unsigned int j = 0; j < npar.size(); j++) {
        // Covariance terms
        dlnl += npar.at(i) * InvErrorMatrix->GetBinContent(i + 1, j + 1) *
                npar.at(j);
        std::cout << Form("%5.3f  ", InvErrorMatrix->GetBinContent(i + 1, j + 1));
      }
      std::cout << std::endl;
    }
  }
...
void NueFit2D::FormatChi2Hists(int nx, double xlow, double xhigh, int ny,
...
void NueFit2D::SetupChi2Hists() {
  Chi2_Normal =
      new TH2D("Chi2_Normal", "", nSinSq2Th13_Chi2Hist, SinSq2Th13Low_Chi2Hist,
               SinSq2Th13High_Chi2Hist, nDelta_Chi2Hist, DeltaLow_Chi2Hist,
               DeltaHigh_Chi2Hist);
  Chi2_Inverted =
      new TH2D("Chi2_Inverted", "", nSinSq2Th13_Chi2Hist,
               SinSq2Th13Low_Chi2Hist, SinSq2Th13High_Chi2Hist, nDelta_Chi2Hist,
               DeltaLow_Chi2Hist, DeltaHigh_Chi2Hist);
...
void NueFit2D::RunStandardChi2Sensitivity() {
  ...
  for (j = 0; j < nDeltaSteps + 1; j++) {
    ...
    for (i = 0; i < nSinSq2Th13Steps + 1; i++) {
      ...
      if (chi2 > contourlvl && chi2prev < contourlvl) {
        best = ss2th13 +
               ((ss2th13 - prev) / (chi2 - chi2prev)) * (contourlvl - chi2);
        limit90_norm.push_back(best);
        deltapts_norm.push_back(delta / TMath::Pi());
        cout << chi2prev << ", " << prev << ", " << chi2 << ", " << ss2th13
             << ", " << best << endl;
        break;
      }
      chi2prev = chi2;
  ...
  cout << "90% confidence level limit = " << limit90_norm.at(0) << ", "
       << limit90_invt.at(0) << endl;
...
void NueFit2D::RunFCAnalytical() {
  ...
  if (nBins > 1) {
    cout << "Analytical Feldman-Cousins Method should only be used for a "
            "single bin fit.  Quitting..."
         << endl;
    return;
  }
  ...
  for (i = 0; i < nPts_Normal; i++) {
    if (i % 100 == 0) {
      cout << 100. * i / nPts_Normal << "% complete for normal hierarchy"
           << endl;
    }
    GridTree_Normal[0]->GetEntry(i);
    omega = EvaluateOmega(grid_signal * GridScale_Normal,
                          grid_background * GridScale_Normal);
    Chi2_Normal->Fill(grid_sinsq2th13, grid_delta / TMath::Pi(), omega);
  }

  for (i = 0; i < nPts_Inverted; i++) {
    if (i % 100 == 0) {
      cout << 100. * i / nPts_Inverted << "% complete for inverted hierarchy"
           << endl;
    }
    GridTree_Inverted[0]->GetEntry(i);
    omega = EvaluateOmega(grid_signal * GridScale_Inverted,
                          grid_background * GridScale_Inverted);
...
void NueFit2D::RunFCTraditional() {
  ...
  bool calcchi2min; // if nobs<nexpmin for any bin, then chi2min!=0 and needs
                    // to be calculated
  TH1D *nexpmin = 0;
...
void NueFit2D::RunMultiBinPseudoExpts(bool Print) {
  ...
  TH2D *Error4Expts = new TH2D("Error4Expts", "", nBins, -0.5, nBins - 0.5,
                               nBins, -0.5, nBins - 0.5);
  ...
  for (i = 0; i < nPts_Normal; i++) {
    cout << "point " << (i + 1) << "/" << nPts_Normal << " (normal hierarchy)"
         << endl;
    ...
    if (IncludeOscParErrs) {
      ...
      for (j = 0; j < nBins; j++) {
        ...
        ele += (grid_bin_oscparerr[j] * grid_bin_oscparerr[j] *
                nexp->GetBinContent(j + 1) * nexp->GetBinContent(j + 1));
        Error4Expts->SetBinContent(j + 1, j + 1, ele);

        for (k = 0; k < nBins; k++) {
          if (k > j) {
            ele = Error4Expts->GetBinContent(j + 1, k + 1);
            ele += (grid_bin_oscparerr[j] * grid_bin_oscparerr[k] *
                    nexp->GetBinContent(j + 1) * nexp->GetBinContent(k + 1));
    ...
    for (u = 0; u < NumExpts; u++) {
      ...
      if (FitMethod == 0) {
        ...
        cout << "Error in RunMultiBinPseudoExpts(): Unknown 'FitMethod'."
             << endl;
      }
      chi2hist->Fill(delchi2);
    }
...
void NueFit2D::GenerateOneExperiment(TH1D *nexp_bkgd, TH1D *nexp_signal,
                                     TH1D *dnexp_oscpar) {
...
void NueFit2D::GenerateOneCorrelatedExp(TH1D *nexp, TH2D *err) {
  ...
  vector<double> nrand;
  for (i = 0; i < nBins; i++) {
    //     cout<<"nexp_ = "<<nexp_->GetBinContent(i+1)<<"
    //     "<<sqrt(D[i][i])<<endl;
    if (D[i][i] < 0) {
      cout
          << "Warning in NueFit2D::GenerateOneCorrelatedExp(): Negative "
             "element in diagonalized systematic error matrix ... setting to 0."
          << endl;
      D[i][i] = 0;
    }
  ...
  for (i = 0; i < nBins; i++) {
    ...
    // TESTING SYSTEMATICS: Jun 25 2020
...
void NueFit2D::CalculateErrorMatrixInv(TH1D *nexp) {
  ...
  if (ErrCalc != 0) { // if ErrorCalc object has been added, use it to
                      // calculate covariance matrix
    ErrCalc->CalculateSystErrorMatrix();
    ErrorMatrix->Add(ErrCalc->CovMatrix);
    ErrCalc->CalculateHOOError();
    ErrorMatrix->Add(ErrCalc->CovMatrix_Decomp);
  } else if (ExternalErrorMatrix !=
             0) { // if setting a systematic matrix externally, use it
...
double NueFit2D::EvaluateOmega(double signal, double background) {
  ...
  if (errK < 1e-5) {
    errK = 1.0;
    std::cout << "can't do perfect signal" << std::endl;
  }
  ...
  //   std::cout<<n0<<"  "<<s0<<"  "<<b0<<"  "<<k0<<"  "<<betaHat<<" "<<kHat<<"
  //   "<<errBg<<" "<<errK<<std::endl; std::cout<<s0*kHat + betaHat<<"
  //   "<<n0<<std::endl;
  ...
  /*   so lets be clear about this, the values for these gaussians are the same
     but its the rank that changes for any given value of n (the values
     themselve differ with mu) so the first time through i calculate the
     contribution for each point in the space then when looping through just
     look it up in a giant array             */

  // better yet - I can save cycles by building the arrays separately on the
  // first pass
  ...
  while (delta > ThreshHold || (1 - (omega + omegaBar) > 2 * ThreshHold)) {
    ...
    if (doneH && doneL) {
      // just a sanity check that the code doesn't think its done too early,
      // this will cause an inf loop
      if (1 - (omega + omegaBar) < 1.5 * slip) {
        break;
      }
      std::cout << "Thats unexpected:  " << 1 - (omega + omegaBar) << "  "
                << slip << std::endl;
    }

    if (PrefactorH > pfThresh ||
        PrefactorL > pfThresh) { // No need to loop if contribution too small
    ...
    delta = TMath::Max(PrefactorH * dOmegaH + PrefactorL * dOmegaL,
                       PrefactorH * dOmegaBarH + PrefactorL * dOmegaBarL);
  ...
  if (slip > ThreshHold) {
    std::cout << "Integral error past threshold:  " << omega << "  " << omegaBar
              << "  " << omega + omegaBar << "  " << slip << std::endl;
    std::cout << n0 << "  " << s0 << "  " << b0 << "  " << k0 << "  " << betaHat
              << " " << kHat << "  " << errBg << " " << errK << std::endl;
    std::cout << s0 * kHat + betaHat << "  " << n0 << std::endl;
  }

  return omega;
}
double NueFit2D::CalculateRank(int n, double s, double b, double errBg,
                               double k, double errK) {
  double results[3];
  ...
  if (n >= b) { // then we have n = sk+b, Beta = b; k = k0// don't have to do
                // anything
    sBest = (n - b) / k;
    kBest = k;
    bBest = b;
    ...
    bBest = 0.5 * (b - errBg +
                   TMath::Sqrt((b - errBg) * (b - errBg) + 4 * n * errBg));
  ...
  double betaHat = results[0];
  double kHat = results[1];
  double logN = n * TMath::Log(s * kHat + betaHat) - (s * kHat + betaHat) -
                (betaHat - b) * (betaHat - b) / (2 * errBg) -
                (kHat - k) * (kHat - k) / (2 * errK);
  double logD = n * TMath::Log(sBest * kBest + bBest) -
                (sBest * kBest + bBest) -
                (bBest - b) * (bBest - b) / (2 * errBg) -
...
bool NueFit2D::FindBestFit(int n, double s, double b, double errBg, double k,
                           double errK, double *res) {
  //  At the moment this function is solvable in ~closed form (we ignore the
  //  dependance of errBg, errK on khat, betaHat this lets us avoid having a
  //  numerical minimization, but the second half of this function is the code
  //  that allows for that

  double sol_A = 1 + errK / errBg * s * s;
  double sol_B =
      errBg + s * k - 2 * errK * s * s * b / errBg - b + s * s * errK;
  double sol_C = -n * errBg + s * k * errBg - s * s * errK * b - s * k * b +
                 s * s * b * b * errK / errBg;
  ...
  /*
     ...
     fFixedFitPar[5] = errK;
   */
  //   res[2] = fResult = Poisson(s*kHat+betaHat, n, true)*Gaussian(betaHat, b,
  //   errBg)*Gaussian(kHat, k, errK);
  /*
     static bool first = true;
...
double NueFit2D::GetSensitivityAt(double delta, bool normalhier) {
  ...
  for (i = 0; i < nSinSq2Th13Steps + 1; i++) {
    ...
    if (i < 3 && chi2[2] > chi2[1] &&
        chi2[1] > chi2[0]) { // first three points are increasing, first point
                             // is minimum.
      best = val[0];
      minchi2 = chi2[0];
    ...
    if (chi2[2] > chi2[1] && chi2[0] > chi2[1]) { // found minimum
      ...
      if (fit->GetParameter(2) > 0) { // if the x^2 term is nonzero
        best = -fit->GetParameter(1) /
               (2 * fit->GetParameter(
                        2)); // the location of the minimum is -p1/(2*p2)
        minchi2 = fit->GetParameter(0) + fit->GetParameter(1) * best +
                  fit->GetParameter(2) * best * best;
        cout << "minimum with fit: " << minchi2 << " at " << best << endl;
      } else { // if the x^2 term is zero, then just use the minimum you got by
               // scanning
        best = val[1];
        minchi2 = chi2[1];
  ...
  for (i = 0; i < nSinSq2Th13Steps + 1; i++) {
    ...
    if (delchi2 > contourlvl && delchi2prev < contourlvl &&
        TMath::Abs(delchi2prev - delchi2) < 0.1) {
      limit = ss2th13 + ((ss2th13 - prev) / (delchi2 - delchi2prev)) *
                            (contourlvl - delchi2);
      sens = limit;
      break;
    }
...
void NueFit2D::RunDeltaChi2Contour(int cl) {
  ...
  if (cl == 0) { // 90% CL
    ...
    cout << "Error in RunDeltaChi2Contour(): Input value should be 0 or 1 for "
            "90% or 68.3%.  Quitting..."
         << endl;
    return;
  }
  ...
  for (j = 0; j < nDeltaSteps + 1; j++) {
    ...
    for (i = 0; i < nSinSq2Th13Steps + 1; i++) {
      ...
      if (i < 3 && chi2[2] > chi2[1] &&
          chi2[1] > chi2[0]) { // first three points are increasing, first
                               // point is minimum.
        best = val[0];
        minchi2 = chi2[0];
        cout << "minimum at 1st point: " << minchi2 << " at " << best << endl;
      ...
      if (chi2[2] > chi2[1] && chi2[0] > chi2[1]) { // found minimum
        ...
        if (fit->GetParameter(2) > 0) { // if the x^2 term is nonzero
          ...
          minchi2 = fit->GetParameter(0) + fit->GetParameter(1) * best +
                    fit->GetParameter(2) * best * best;
          cout << "minimum with fit: " << minchi2 << " at " << best << endl;
        } else { // if the x^2 term is zero, then just use the minimum you got
                 // by scanning
          best = val[1];
          minchi2 = chi2[1];
          cout << "minimum with scan: " << minchi2 << " at " << best << endl;
    ...
    for (i = 0; i < (SinSq2Th13High - best) / Th13increment; i++) {
      ...
      if (delchi2 > contourlvl && delchi2prev < contourlvl) {
        limit = ss2th13 + ((ss2th13 - sprev) / (delchi2 - delchi2prev)) *
                              (contourlvl - delchi2);
        cout << delchi2prev << ", " << sprev << ", " << delchi2 << ", "
             << ss2th13 << ", " << limit << endl;
        break;
      }
      delchi2prev = delchi2;
  ...
  cout << "confidence level limit = " << limit_norm.at(0) << ", "
       << limit_invt.at(0) << endl;
...
double NueFit2D::GetLikelihood(double t12, double t23, double t13,
                               double dm2_32, double dm2_21, double delta) {
  if (NObs == 0) {
    cout << "NObs not set.  Quitting..." << endl;
    return 0;
  }
  ...
  for (i = 0; i < nPts_Normal; i++) {
    ...
    if (IncludeOscParErrs) {
      ...
      for (j = 0; j < nBins; j++) {
        ...
        ele = Error4Expts->GetBinContent(j + 1, j + 1);
        ele += (grid_bin_oscparerr[j] * grid_bin_oscparerr[j] *
                nexp_n->GetBinContent(j + 1) * nexp_n->GetBinContent(j + 1));
        Error4Expts->SetBinContent(j + 1, j + 1, ele);
        ...
        for (k = 0; k < nBins; k++) {
          if (k > j) {
            ele = Error4Expts->GetBinContent(j + 1, k + 1);
            ele +=
                (grid_bin_oscparerr[j] * grid_bin_oscparerr[k] *
                 nexp_n->GetBinContent(j + 1) * nexp_n->GetBinContent(k + 1));
            Error4Expts->SetBinContent(j + 1, k + 1, ele);
    ...
    for (u = 0; u < NumExpts; u++) {
      ...
      if (FitMethod == 0) {
        ...
        cout << "Error in RunMultiBinPseudoExpts_MHDeltaFit(): Unknown "
                "'FitMethod'."
             << endl;
      }
      ...
      if (Print) {
        myfile << chi2_norm << " " << chi2_invt << " " << chi2min_norm << " "
               << chi2min_invt << endl;
      }
  ...
  for (i = 0; i < nPts_Inverted; i++) {
    ...
    if (IncludeOscParErrs) {
      ...
      for (j = 0; j < nBins; j++) {
        ...
        ele = Error4Expts->GetBinContent(j + 1, j + 1);
        ele += (grid_bin_oscparerr[j] * grid_bin_oscparerr[j] *
                nexp_i->GetBinContent(j + 1) * nexp_i->GetBinContent(j + 1));
        Error4Expts->SetBinContent(j + 1, j + 1, ele);
        ...
        for (k = 0; k < nBins; k++) {
          if (k > j) {
            ...
            ele +=
                (grid_bin_oscparerr[j] * grid_bin_oscparerr[k] *
                 nexp_i->GetBinContent(j + 1) * nexp_i->GetBinContent(k + 1));
            Error4Expts->SetBinContent(j + 1, k + 1, ele);
...
void NueFit2D::RunMultiBinFC_MHDeltaFit() {
  ...
  for (i = 0; i < nPts_Normal; i++) {
    ...
    if (FitMethod == 0) {
      ...
      cout << "Error in RunMultiBinFC_MHDeltaFit(): Unknown 'FitMethod'."
           << endl;
    }
  ...
  for (i = 0; i < nPts_Normal; i++) {
    ...
    cout << "NH: chi2min = " << chi2min_NH[i]
         << ", IH: chi2min = " << chi2min_IH[i] << endl;
...
void NueFit2D::RunSterileContour(int cl) {
  ...
  for (j = 0; j < 19; j++) {
    ...
    for (i = 0; i < nSinSq2Th14Steps + 1; i++) {
      ...
      if (delchi2 > contourlvl && delchi2prev < contourlvl) {
        limit = (ss2th14 + ((ss2th14 - sprev) / (delchi2 - delchi2prev)) *
                               (contourlvl - delchi2));
        cout << delchi2prev << ", " << sprev << ", " << delchi2 << ", "
             << ss2th14 << ", " << limit << endl;
...
void NueFit2D::Run3FDeltaCP(unsigned int nDeltaCPSteps) {
  DefineBinDlnLMinuit();
  ...
  int arraysize = (nDeltaCPSteps + 1);
  double* dcp = (double*) malloc(arraysize * sizeof(double));
  double* chi = (double*) malloc(arraysize * sizeof(double));
  double* dchi = (double*) malloc(arraysize * sizeof(double));
  double mc2;

  Double_t DeltaCPincrement = 0;
  if (nDeltaCPSteps > 0) {
    DeltaCPincrement = 2 * TMath::Pi() / (nDeltaCPSteps);
  }

  for (unsigned int i = 0; i < nDeltaCPSteps + 1; i++) {
    double deltaCP = i * DeltaCPincrement;
    dcp[i] = deltaCP;

    for (unsigned int ie = 0; ie < Extrap.size(); ie++) {
      Extrap[ie]->SetOscPar(OscPar::kDelta, deltaCP);
      Extrap[ie]->OscillatePrediction();
    }
    ...
    std::cout << "Param before fit: " << 3 << std::endl;
    chi[i] = BinLikelihood();
  }
  ...
  TGraph *bpss = new TGraph(arraysize, dcp, dchi);
  bpss->SetName("dchi");
  bpss->Write();
...
void NueFit2D::Run2DSterileSlice_LogScale(bool DoCombineFit, bool lowEfit) {
  mDoCombineFit = DoCombineFit;
  ...
  const unsigned int N_ssqth14 = 50 + 1; // nSinSqTh14Steps + 1;
  double lin10vals_ssqth14[N_ssqth14];
  double rangeLog10_ssqth14_low = TMath::Log10(SinSqTh14Low == 0. ? 1E-3 : SinSqTh14Low);
  double rangeLog10_ssqth14_high = TMath::Log10(SinSqTh14High == 0. ? 1 : SinSqTh14High);
  double stepLog10_ssqth14 = (rangeLog10_ssqth14_high - rangeLog10_ssqth14_low) / (double)(N_ssqth14 - 1);
  for (unsigned int i = 0; i < N_ssqth14; i++) {
    lin10vals_ssqth14[i] = TMath::Power(10, rangeLog10_ssqth14_low + (double) i * stepLog10_ssqth14);
  }

  const unsigned int N_ssqth24 = 50 + 1; // nSinSqTh24Steps + 1;
  double lin10vals_ssqth24[N_ssqth24];
  double rangeLog10_ssqth24_low = TMath::Log10(SinSqTh24Low == 0. ? 1E-3 : SinSqTh24Low);
  double rangeLog10_ssqth24_high = TMath::Log10(SinSqTh24High == 0. ? 1 : SinSqTh24High);
  double stepLog10_ssqth24 = (rangeLog10_ssqth24_high - rangeLog10_ssqth24_low) / (double)(N_ssqth24 - 1);
  for (unsigned int i = 0; i < N_ssqth24; i++) {
    lin10vals_ssqth24[i] = TMath::Power(10, rangeLog10_ssqth24_low + (double) i * stepLog10_ssqth24);
  }
  ...
  for (unsigned int i = 0; i < N_ssqth14; i++) {
    ...
    double Theta14 = TMath::ASin(TMath::Sqrt(ssth14));
    double csth14 = TMath::Cos(Theta14) * TMath::Cos(Theta14);
    ...
    for (unsigned int j = 0; j < N_ssqth24; j++) {
      ...
      if (FitMethod == 0) {
        ...
        if (DoCombineFit) {
          chi_combine[idx] = chi[idx] + disappChi2;
        }
      } else {
  ...
  for (int s = 0; s < arraysize; s++) {
    ...
    if (abs(dchi_combine[s]) < 0.00001) {
      cout << "I have a zero for combined fit!" << endl;
    }
  }
...
void NueFit2D::Run2DSterileSlice(bool DoCombineFit) {
  mDoCombineFit = DoCombineFit;
  // Given a value of Dm41, produces a 2D chisquared graph of both sin(th14)^2
  // vs sin(th24)^2 and |Ue4|^2 vs |Umu4|^2
...
void NueFit2D::Run2DSterileSlice_WithPulls(bool DoCombineFit) {
  mDoCombineFit = DoCombineFit;
  // Given a value of Dm41, produces a 2D chisquared graph of both sin(th14)^2
  // vs sin(th24)^2 and |Ue4|^2 vs |Umu4|^2
  ...
  double Ssqth24[10201];
  double chi[10201];
  double chi_syst[10201];
  double chi_stat[10201];
  double chi_numu[10201];
  double dchi[10201];
  double systPullTerm_bin1[10201];
  double systPullTerm_bin2[10201];
  double systPullTerm_bin3[10201];
  double systPullTerm_bin4[10201];
  double systPullTerm_bin5[10201];
  double systPullTerm_bin6[10201];
  double systPullTerm_bin7[10201];
  double systPullTerm_bin8[10201];
  double systPullTerm_bin9[10201];
  double systPullTerm_bin10[10201];
  double systPullTerm_bin11[10201];
  double systPullTerm_bin12[10201];
  ...
  for (i = 0; i < nSinSqTh14Steps + 1; i++) {
    ...
    for (j = 0; j < nSinSqTh24Steps + 1; j++) {
      ...
      if (FitMethod == 0) {
        ...
        double chi2_syst = 0.;
        double chi2_stat = 0.;
        std::vector<double> npar_bestfit; npar_bestfit.clear();
        chi[idx] = BinLikelihood_WithPulls(chi2_syst, chi2_stat, npar_bestfit);
        systPullTerm_bin1[idx] = npar_bestfit.at(0);
        systPullTerm_bin2[idx] = npar_bestfit.at(1);
        systPullTerm_bin3[idx] = npar_bestfit.at(2);
        systPullTerm_bin4[idx] = npar_bestfit.at(3);
        systPullTerm_bin5[idx] = npar_bestfit.at(4);
        systPullTerm_bin6[idx] = npar_bestfit.at(5);
        systPullTerm_bin7[idx] = npar_bestfit.at(6);
        systPullTerm_bin8[idx] = npar_bestfit.at(7);
        systPullTerm_bin9[idx] = npar_bestfit.at(8);
        systPullTerm_bin10[idx] = npar_bestfit.at(9);
        systPullTerm_bin11[idx] = npar_bestfit.at(10);
        systPullTerm_bin12[idx] = npar_bestfit.at(11);
        chi_syst[idx] = chi2_syst;
        chi_stat[idx] = chi2_stat;
        chi_numu[idx] = disappChi2;
      } else {
        cout << "Error in Run2DSterileSlice(): Unknown 'FitMethod'." << endl;
      }
  ...
  double param_idx[4] = {1., 2., 3., 4.};
  double param_val[4] = {disapp_delta13, disapp_delta14, disapp_delta24, disapp_th34};
  TGraph *gr_grid_param = new TGraph(4, param_idx, param_val);
  gr_grid_param->SetName("gr_grid_param");
  gr_grid_param->Write();
  ...
  TGraph2D *gr_systPullTerm_bin1 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin1);
  gr_systPullTerm_bin1->SetName("systPullTerm_bin1");
  gr_systPullTerm_bin1->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin1->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin1->GetZaxis()->SetTitle("systPullTerm_bin1");
  gr_systPullTerm_bin1->Write();

  TGraph2D *gr_systPullTerm_bin2 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin2);
  gr_systPullTerm_bin2->SetName("systPullTerm_bin2");
  gr_systPullTerm_bin2->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin2->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin2->GetZaxis()->SetTitle("systPullTerm_bin2");
  gr_systPullTerm_bin2->Write();

  TGraph2D *gr_systPullTerm_bin3 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin3);
  gr_systPullTerm_bin3->SetName("systPullTerm_bin3");
  gr_systPullTerm_bin3->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin3->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin3->GetZaxis()->SetTitle("systPullTerm_bin3");
  gr_systPullTerm_bin3->Write();

  TGraph2D *gr_systPullTerm_bin4 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin4);
  gr_systPullTerm_bin4->SetName("systPullTerm_bin4");
  gr_systPullTerm_bin4->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin4->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin4->GetZaxis()->SetTitle("systPullTerm_bin4");
  gr_systPullTerm_bin4->Write();

  TGraph2D *gr_systPullTerm_bin5 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin5);
  gr_systPullTerm_bin5->SetName("systPullTerm_bin5");
  gr_systPullTerm_bin5->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin5->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin5->GetZaxis()->SetTitle("systPullTerm_bin5");
  gr_systPullTerm_bin5->Write();

  TGraph2D *gr_systPullTerm_bin6 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin6);
  gr_systPullTerm_bin6->SetName("systPullTerm_bin6");
  gr_systPullTerm_bin6->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin6->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin6->GetZaxis()->SetTitle("systPullTerm_bin6");
  gr_systPullTerm_bin6->Write();

  TGraph2D *gr_systPullTerm_bin7 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin7);
  gr_systPullTerm_bin7->SetName("systPullTerm_bin7");
  gr_systPullTerm_bin7->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin7->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin7->GetZaxis()->SetTitle("systPullTerm_bin7");
  gr_systPullTerm_bin7->Write();

  TGraph2D *gr_systPullTerm_bin8 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin8);
  gr_systPullTerm_bin8->SetName("systPullTerm_bin8");
  gr_systPullTerm_bin8->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin8->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin8->GetZaxis()->SetTitle("systPullTerm_bin8");
  gr_systPullTerm_bin8->Write();

  TGraph2D *gr_systPullTerm_bin9 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin9);
  gr_systPullTerm_bin9->SetName("systPullTerm_bin9");
  gr_systPullTerm_bin9->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin9->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin9->GetZaxis()->SetTitle("systPullTerm_bin9");
  gr_systPullTerm_bin9->Write();

  TGraph2D *gr_systPullTerm_bin10 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin10);
  gr_systPullTerm_bin10->SetName("systPullTerm_bin10");
  gr_systPullTerm_bin10->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin10->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin10->GetZaxis()->SetTitle("systPullTerm_bin10");
  gr_systPullTerm_bin10->Write();

  TGraph2D *gr_systPullTerm_bin11 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin11);
  gr_systPullTerm_bin11->SetName("systPullTerm_bin11");
  gr_systPullTerm_bin11->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin11->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin11->GetZaxis()->SetTitle("systPullTerm_bin11");
  gr_systPullTerm_bin11->Write();

  TGraph2D *gr_systPullTerm_bin12 = new TGraph2D(arraysize, Ue42, Ssqth24, systPullTerm_bin12);
  gr_systPullTerm_bin12->SetName("systPullTerm_bin12");
  gr_systPullTerm_bin12->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_systPullTerm_bin12->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_systPullTerm_bin12->GetZaxis()->SetTitle("systPullTerm_bin12");
  gr_systPullTerm_bin12->Write();

  TGraph2D *ss_syst = new TGraph2D(arraysize, Ue42, Ssqth24, chi_syst);
  ss_syst->SetName("ss_syst");
  ss_syst->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  ss_syst->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  ss_syst->GetZaxis()->SetTitle("-2lnL (syst)");
  ss_syst->Write();

  TGraph2D *ss_stat = new TGraph2D(arraysize, Ue42, Ssqth24, chi_stat);
  ss_stat->SetName("ss_stat");
  ss_stat->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  ss_stat->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  ss_stat->GetZaxis()->SetTitle("-2lnL (stat)");
  ss_stat->Write();

  TGraph2D *ss_numu = new TGraph2D(arraysize, Ue42, Ssqth24, chi_numu);
  ss_numu->SetName("ss_numu");
  ss_numu->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  ss_numu->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  ss_numu->GetZaxis()->SetTitle("-2lnL (numu)");
  ss_numu->Write();
...
void NueFit2D::Run2DSterileSlice_Separate_Syst_Stat(bool DoCombineFit) {
  mDoCombineFit = DoCombineFit;
  // Given a value of Dm41, produces a 2D chisquared graph of both sin(th14)^2
  // vs sin(th24)^2 and |Ue4|^2 vs |Umu4|^2
  ...
  if (FitMethod == 4) {
    DefineBinDlnLMinuit_Separate_Syst_Stat();
  }
  ...
  double systPullTerm_bin11[10201];
  double systPullTerm_bin12[10201];
  double bestfit_th13[10201];
  double bestfit_th23[10201];
  double bestfit_dm32[10201];
  ...
  for (i = 0; i < nSinSqTh14Steps + 1; i++) {
    ...
    for (j = 0; j < nSinSqTh24Steps + 1; j++) {
      ...
      if (FitMethod == 0) {
        ...
        double chi2_numu = 0.;
        std::vector<double> npar_bestfit; npar_bestfit.clear();
        chi[idx] = BinLikelihood_Separate_Syst_Stat(chi2_syst, chi2_stat, chi2_numu, npar_bestfit);
        systPullTerm_bin1[idx] = npar_bestfit.at(0);
        systPullTerm_bin2[idx] = npar_bestfit.at(1);
        systPullTerm_bin3[idx] = npar_bestfit.at(2);
        systPullTerm_bin4[idx] = npar_bestfit.at(3);
        ...
        systPullTerm_bin11[idx] = npar_bestfit.at(10);
        systPullTerm_bin12[idx] = npar_bestfit.at(11);
        bestfit_th13[idx] = npar_bestfit.at(12);
        bestfit_th23[idx] = npar_bestfit.at(13);
        bestfit_dm32[idx] = npar_bestfit.at(14);
        chi_syst[idx] = chi2_syst;
        chi_stat[idx] = chi2_stat;
        chi_numu[idx] = chi2_numu;
      } else {
        cout << "Error in Run2DSterileSlice(): Unknown 'FitMethod'." << endl;
      }
  ...
  TGraph2D *gr_bestfit_th13 = new TGraph2D(arraysize, Ue42, Ssqth24, bestfit_th13);
  gr_bestfit_th13->SetName("bestfit_th13");
  gr_bestfit_th13->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_bestfit_th13->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_bestfit_th13->GetZaxis()->SetTitle("bestfit_th13");
  gr_bestfit_th13->Write();

  TGraph2D *gr_bestfit_th23 = new TGraph2D(arraysize, Ue42, Ssqth24, bestfit_th23);
  gr_bestfit_th23->SetName("bestfit_th23");
  gr_bestfit_th23->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_bestfit_th23->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_bestfit_th23->GetZaxis()->SetTitle("bestfit_th23");
  gr_bestfit_th23->Write();

  TGraph2D *gr_bestfit_dm32 = new TGraph2D(arraysize, Ue42, Ssqth24, bestfit_dm32);
  gr_bestfit_dm32->SetName("bestfit_dm32");
  gr_bestfit_dm32->GetXaxis()->SetTitle("sin^{2}#theta_{14}");
  gr_bestfit_dm32->GetYaxis()->SetTitle("sin^{2}#theta_{24}");
  gr_bestfit_dm32->GetZaxis()->SetTitle("bestfit_dm32");
  gr_bestfit_dm32->Write();
...
void NueFit2D::Run2DSterileSlice_Separate_Syst_Stat_LogScale(bool DoCombineFit) {
  mDoCombineFit = DoCombineFit;
  ...
  if (FitMethod == 4) DefineBinDlnLMinuit_Separate_Syst_Stat_LogScale();
  ...
  const unsigned int N_ssqth14 = 2; //50; // nSinSqTh14Steps + 1;
  double lin10vals_ssqth14[N_ssqth14 + 1];
  double rangeLog10_ssqth14_low = TMath::Log10(SinSqTh14Low == 0. ? 1E-4 : SinSqTh14Low);
  double rangeLog10_ssqth14_high = TMath::Log10(SinSqTh14High == 0. ? 1 : SinSqTh14High);
  double stepLog10_ssqth14 = (rangeLog10_ssqth14_high - rangeLog10_ssqth14_low) / (double)N_ssqth14;
  for (unsigned int i = 0; i < N_ssqth14 + 1; i++) lin10vals_ssqth14[i] = TMath::Power(10, rangeLog10_ssqth14_low + (double) i * stepLog10_ssqth14);

  const unsigned int N_ssqth24 = 2; //50; // nSinSqTh24Steps + 1;
  double lin10vals_ssqth24[N_ssqth24 + 1];
  double rangeLog10_ssqth24_low = TMath::Log10(SinSqTh24Low == 0. ? 1E-4 : SinSqTh24Low);
  double rangeLog10_ssqth24_high = TMath::Log10(SinSqTh24High == 0. ? 1 : SinSqTh24High);
  double stepLog10_ssqth24 = (rangeLog10_ssqth24_high - rangeLog10_ssqth24_low) / (double)N_ssqth24;
  for (unsigned int i = 0; i < N_ssqth24 + 1; i++) lin10vals_ssqth24[i] = TMath::Power(10, rangeLog10_ssqth24_low + (double) i * stepLog10_ssqth24);

  Int_t idx = 0; // Array indexer
  for (unsigned int i = 0; i < N_ssqth14 + 1; i++) {
    Double_t ssth14 = lin10vals_ssqth14[i];
    double Theta14 = TMath::ASin(TMath::Sqrt(ssth14));
    Double_t csth14 = TMath::Cos(Theta14) * TMath::Cos(Theta14);

    for (unsigned int ie = 0; ie < Extrap.size(); ie++) Extrap[ie]->SetSinSqTh14(ssth14);

    for (unsigned int j = 0; j < N_ssqth24 + 1; j++) {
      Double_t ssth24 = lin10vals_ssqth24[j];
      ...
      if (FitMethod == 0) {
        ...
        // Likelihood: Bin by Bin Calculation of Systematics: Calculate the likelihood (x2 for chi)
        Ue42[idx] = ssth14;
        Um42[idx] = csth14 * ssth24;
        Ssqth24[idx] = ssth24;
        double chi2_syst = 0.;
        ...
        double chi2_numu = 0.;
        std::vector<double> npar_bestfit; npar_bestfit.clear();
        std::cout << "Before the fit i = " << i << ", j = " << j << "." << std::endl;
        chi[idx] = BinLikelihood_Separate_Syst_Stat_LogScale(chi2_syst, chi2_stat, chi2_numu, npar_bestfit);
        std::cout << "After the fit i = " << i << ", j = " << j << "." << std::endl;
        systPullTerm_bin1[idx] = npar_bestfit.at(0);
        systPullTerm_bin2[idx] = npar_bestfit.at(1);
        systPullTerm_bin3[idx] = npar_bestfit.at(2);
        systPullTerm_bin4[idx] = npar_bestfit.at(3);
  ...
  double mc2 = 100000.;
  unsigned int arraysize = (N_ssqth14 + 1) * (N_ssqth24 + 1);
  ...
  for (unsigned int s = 0; s < arraysize; s++) {
    dchi[s] = chi[s] - mc2;
    if (abs(dchi[s]) < 0.00001) {
      cout << "I have a zero!" << endl;
    }
  ...
  double param_idx[7] = {1., 2., 3., 4., 5., 6., 7.};
  double disapp_delta13 = (Extrap.at(0)->osc).GetOscParam(OscPar::kDelta);
  double disapp_delta14 = (Extrap.at(0)->osc).GetOscParam(OscPar::kDelta14);
  double disapp_delta24 = (Extrap.at(0)->osc).GetOscParam(OscPar::kDelta24);
  double disapp_th34    = (Extrap.at(0)->osc).GetOscParam(OscPar::kTh34);
  ...
  double disapp_th23 = (Extrap.at(0)->osc).GetOscParam(OscPar::kTh23);
  double disapp_dm2_32  = (Extrap.at(0)->osc).GetOscParam(OscPar::kDeltaM23);
  double param_val[7] = {disapp_delta13, disapp_delta14, disapp_delta24, disapp_th34, disapp_th13, disapp_th23, disapp_dm2_32};
  TGraph *gr_grid_param = new TGraph(7, param_idx, param_val);
  gr_grid_param->SetName("gr_grid_param");
  gr_grid_param->Write();
...
void NueFit2D::RunSterileFit() {
  ...
  TH1D *nexp_signal_min =
      new TH1D("nexp_signal_min", "", nBins, -0.5, nBins - 0.5);
  ...
  TH1D *err = new TH1D("err", "", 1, -0.5, 0.5);
  err->SetBinContent(1, 0.05);
  TH1D *sigerr = new TH1D("sigerr", "", 1, -0.5, 0.5);
  sigerr->SetBinContent(1, 0.05);
 ...
 */
/*
void NueFit2D::RunMultiBinPseudoExptsSterileFit(bool DoCombineFit, bool Print, int profileIndex,
                                                int fileIndex) {
  if (FitMethod == 1 && (FracErr_Bkgd == 0 || FracErr_Sig == 0)) {
    cout << "FracErr_Bkgd and FracErr_Sig need to be set for ScaledChi2.  "
            "Quitting..."
  ...
  TH1D *DeltaChi2Hist =
      new TH1D("DeltaChi2Hist", "DeltaChi2Hist", 25001, -50, 200);
  // for (int i = profileIndex; i < profileIndex + 5000; i++) {
  for (int i = 0; i < 1; i++) {
    // for (int i = profileIndex; i < profileIndex + 50; i++) {
    nexp_bkgd->Reset();
    nexp_signal->Reset();
    nexp->Reset();
    ...
    nexp->Add(nexp_bkgd, nexp_signal, 1, 1);
    // TH1D* modelNevent = new TH1D("modelNevent", "", nBins, -0.5, nBins - 0.5);
    // modelNevent->Reset();
    // modelNevent->Add(nexp_bkgd, nexp_signal, 1, 1);
    ...
    for (unsigned int ibin = 0; ibin < nBins; ibin++) {
      for (unsigned int jbin = 0; jbin < nBins; jbin++) {
        std::cout << Form("%3.4f, ", Error4Expts->GetBinContent(ibin + 1, jbin + 1));
      }
      std::cout << std::endl;
    }
    std::cout << std::endl;
    ...
    double nofit_chi2 = 0;
    std::cout << "nexp mark chi2 = " << nofit_chi2 << ", ";
    for (unsigned int ibin = 0; ibin < nexp->GetSize(); ++ibin) {
      std::cout << Form("%2.4f, ", nexp->GetBinContent(ibin));
    }
    std::cout << std::endl;
    std::cout << std::endl;
    ...
    for (int u = 0; u < 1000; u++) {
      ...
      int ndf = 0;
      int igood = 0;
      nexp->Chi2TestX(NObs, nofit_chi2, ndf, igood);
      std::cout << "No fit chi2 = " << nofit_chi2 << ", ";
      for (unsigned int ibin = 0; ibin < NObs->GetSize(); ++ibin) {
        std::cout << Form("%2.4f, ", NObs->GetBinContent(ibin));
      }
      std::cout << std::endl;

      // vector<double> Start;
      // Start.push_back(grid_sinsqth14);
      // Start.push_back(grid_sinsqth24);
      // Start.push_back(grid_th34);
      // Start.push_back(grid_delta13);
      // Start.push_back(grid_delta14);
      // Start.push_back(grid_delta24);
      // // Global minimum fit
      // chi2min = DoGlobalMinSearchSterileFit(Start, DoCombineFit);

      // double disapp_th14 = TMath::ASin(TMath::Sqrt(grid_sinsqth14));
      // double disapp_th24 = TMath::ASin(TMath::Sqrt(grid_sinsqth24));
      // double disappChi2 = DisappCalc.Chi2(grid_n_dm2_32,
      //                                     grid_n_dm2_21,
      //                                     grid_n_th23,
      //                                     grid_n_th12,
      //                                     grid_n_th13,
      ...
      //     delchi2 = BinLikelihood() - chi2min;
      //   } else {
      //     delchi2 = BinLikelihood() + disappChi2 - chi2min;
      //   }
      // } else {
      //   cout << "Error in RunMultiBinPseudoExpts(): Unknown 'FitMethod'." << endl;
      // }
      // DeltaChi2Hist->Fill(delchi2);
    }
  }

  // std::string identifierString = "";
  // if (!DoCombineFit) {
  //   identifierString = "chi2dist";
  // } else {
  //   identifierString = "chi2dist_combineDisapp";
  // }
  // TFile *outfile = new TFile(
  //     Form("%s_dm41_%.4f_ssq14_%.4f_ssq24_%.4f_%i_%i.root", identifierString.c_str(), grid_dmsq41,
  //          grid_sinsqth14, grid_sinsqth24, fileIndex, profileIndex),
  //     "RECREATE");
  // DeltaChi2Hist->Write();
  // outfile->Write();
  // outfile->Close();
...
nBins, -0.5, nBins - 0.5);
  ...
  for (unsigned int i = 0; i < 11; i++) {
    ...
    for (unsigned int j = 0; j < 11; j++) {
      ...
      for (unsigned int ie = 0; ie < Extrap.size(); ie++) {
        Extrap[ie]->SetOscPar(OscPar::kTh14,
TMath::ASin(TMath::Sqrt(gridSinSTH14))); Extrap[ie]->SetOscPar(OscPar::kTh24,
TMath::ASin(TMath::Sqrt(gridSinSTH24))); Extrap[ie]->OscillatePrediction();
      }
      Bkgd->Reset();
      Sig->Reset();
      NExp->Reset();
      ...
      switch (FitMethod) {
        ...
        default: {
          cout << "Error in GetMinLikelihoodSterileFit(): Unknown 'FitMethod'.
BinLikelihood() is used." << endl; c2 = BinLikelihood(); break;
        }
      }
...
BinLikelihood() is used." << endl; c2 = BinLikelihood(); break;
  ...
  cout << "The min likelihood is " << theLikelihood << " at " << minSinSqTh14 <<
" - " << minSinSqTh24 << endl;
...
double NueFit2D::RunDataSterileFit() {
  if (FitMethod == 4) {
    DefineBinDlnLMinuit();
  }
  ...
  for (ie = 0; ie < Extrap.size(); ie++) {
    Extrap[ie]->SetOscPar(OscPar::kDm41, dm41);
    Extrap[ie]->SetOscPar(OscPar::kTh14, th14);
    Extrap[ie]->SetOscPar(OscPar::kTh24, th24);
    Extrap[ie]->OscillatePrediction();
  }
  Bkgd->Reset();
  Sig->Reset();
