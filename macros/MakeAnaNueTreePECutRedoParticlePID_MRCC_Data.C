// Macro for redoing ParticlePID in previously made  AnaNue trees
//-----------------------------------------------------------------------------
...
void MakeAnaNueTreePECutRedoParticlePID_MRCC_Data(string sntp_file="",string mrnt_file="",string mcnn_file="", string ana_nue_file=""){
  bool isMC=false;
  ...
  ana_nue_file=string(gSystem->ExpandPathName(ana_nue_file.c_str()));
...
//                                "SetKNNModule::Reco "
  ...
  ////////////////////////////////////////////////////////
  //Set Input Streams
  j.Input.Set("Streams=ana_nue,NtpSt,NtpSR,NtpMC,NtpTH,NtpMR,NtpOld,snarltree"); 
  ...
  if(ana_nue_file!="")
  {
    j.Input.AddFile(ana_nue_file.c_str(),"ana_nue");
  }
 ...
 // j.Path("NueAna").Mod("MCNNMergeModule").Config(rr);
  ...
  //copy the pot tree
  sprintf(pottreefn,"POTTreeName=%sAnaNue-%s%s-PECut-%f.root",path,((mcnn_file=="")?"":"mcnn2-"),file.c_str(),pecut);
