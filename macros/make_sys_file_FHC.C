...
void FillHistos(bool isFDTauNue, string nameForStudy, string sysNames, string filelist, SysFileGen2D *sysFile_Fid, SysFileGen2D *sysFile_Pre)
{
  ...
  while( anp.NextEntry() ) //loop over events in ana_nue chain
  {
   ...
   //Some event information can be accessed with anp.isMC(), anp.isFar(), anp.isNC(), etc.   
    ...
    if(anp.isNear())//Near
    {
      eventWeight *= (NueStandard::kNormalizedNearPOT/anp.GetTotalPOT());
    }
    else if(anp.isFar())//Far 
    {
      eventWeight *= ((NueStandard::kNormalizedFarPOT_PerI + NueStandard::kNormalizedFarPOT_PerIII)/anp.GetTotalPOT());
    }
  ...
  if(firstTime)
  {
   ...
   sysFile_Pre->SetFarPOT((NueStandard::kNormalizedFarPOT_PerI + NueStandard::kNormalizedFarPOT_PerIII)*1e12);
   sysFile_Fid->SetFarPOT((NueStandard::kNormalizedFarPOT_PerI + NueStandard::kNormalizedFarPOT_PerIII)*1e12);
