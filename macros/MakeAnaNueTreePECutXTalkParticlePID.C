...
void MakeAnaNueTreePECutXTalkParticlePID(string sntp_file="",string mrnt_file="",string mcnn_file="",int doXTalkFilter=1){
...
//  j.Msg.SetLevel("NueModule","Debug");
 ...
 j.Msg.SetLevel("PETrimmer","Error");
  ...
  //set the analysis path
	...
	if(doXTalkFilter) main_path.PushBack("XTalkFilter","Reco");
 ...
 // j.Path("NueAna").Mod("Output").Cmd("DefineStream  PA PRecord");
  ...
  sprintf(setfilename,"FileName=%sAnaNue-%s%s-PECutXTalk-%f.root",
	  path,((mcnn_file=="")?"":"mcnn2-"),file.c_str(),pecut);
  cout<<"Saving AnaNue Tree to "<<setfilename<<endl;
  j.Path("NueAna").Mod("Output").Set(setfilename);
 

//	kb->SaveSettings(setfilename,"knobs");

 
  
  char pottreefn[1000];
  sprintf(pottreefn,"POTTreeFileName=%sAnaNue-%s%s-PECutXTalk-%f.root",path,((mcnn_file=="")?"":"mcnn2-"),file.c_str(),pecut);
